...
if [ ! -z $RTSP_STREAM ];then
  ...
  while true;do
    for i in ${RTSP_STREAM//,/ };do
      ffmpeg -i  ${i} -t ${RECORDING_LENGTH} -vn -acodec pcm_s16le -ac 2 -ar 256000 file:${RECS_DIR}/StreamData/$(date "+%F")-batnet-$(date "+%H:%M:%S").wav
    done
  done
else
  ...
  if pgrep arecord &> /dev/null ;then
    ...
    if [ -z ${REC_CARD} ];then
      arecord -f S16_LE -c${CHANNELS} -r256000 -t wav --max-file-time ${RECORDING_LENGTH}\
	      --use-strftime ${RECS_DIR}/%B-%Y/%d-%A/%F-batnet-%H-%M-%S.wav
    else
      arecord -f S16_LE -c${CHANNELS} -r256000 -t wav --max-file-time ${RECORDING_LENGTH}\
        -D "${REC_CARD}" --use-strftime \
	${RECS_DIR}/%B-%Y/%d-%A/%F-batnet-%H-%M-%S.wav
    fi
  fi
fi
